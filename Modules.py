...
class ProbMotionEncoderLSTM(nn.Module):
    def __init__(self, in_channels=512, features=[512, 512, 512], kernel_size=(3, 3), padding=(1, 1), activation="relu",
        ...
        self.lstm.add_module("ConvLSTM1",
                             ConvLSTM(in_channels=in_channels, out_channels=features[0], padding=padding,
        ...
        for layer in range(1, len(features)):
            self.lstm.add_module("ConvLSTM{}".format(layer+1),
                                 ConvLSTM(in_channels=features[layer-1], out_channels=features[layer], padding=padding,
                                          kernel_size=kernel_size,  activation=activation, frame_size=frame_size))
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=3, features=[32, 64, 64, 64, 64, 1], kernel_size=3,
                 stride=1, padding=1, pool_size=[2, 2, 2, 4, 2]):
        super(Discriminator, self).__init__()

        self.disc = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=features[0], kernel_size=kernel_size,
                      stride=stride, padding=padding),
            nn.BatchNorm2d(features[0]),
            nn.ReLU(inplace=True),
        )
